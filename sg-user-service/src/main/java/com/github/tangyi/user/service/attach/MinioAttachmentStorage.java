...
public class MinioAttachmentStorage extends AbstractAttachmentStorage {
    ...
    public MinioAttachmentStorage(AttachmentService attachmentService, AttachGroupService groupService,
                                  MinioConfig minioConfig, DefaultImageService defaultImageService,
    ...
    private void initMinioClient() {
        if (StringUtils.isEmpty(minioConfig.getAccessKey())) {
            log.error("The minio accessKey is empty");
            return;
        }

        if (StringUtils.isEmpty(minioConfig.getSecretKey())) {
            log.error("The minio secretKey is empty");
            return;
        }
        try {
            ...
            log.error("Failed to init minioClient", e);
        }
    }
    ...
    @Override
    public ChunkUploadResponse doUploadChunks(File targetFile, String key)
            throws OssException {
        try {
            ChunkUploadContext context = new MinioChunkUploadContext()
            ...
            ObjectWriteResponse res = minioClient.composeObject(ComposeObjectArgs.builder()
                    .bucket(minioConfig.getBucket())
                    .object(key)
                    .sources(sources)
    ...
    public Map<String, Object> doUploadChunk(ChunkUploadContext context, byte[] chunkData, int chunkNumber)
        ...
        try (InputStream in = new ByteArrayInputStream(chunkData)) {
            ...
            log.info("Upload chunk finished, key: {}, chunkName: {}, took: {}ms, etag: {}", context.getKey(),
                    chunkName, took, response.etag());
            return chunkInfo;
        } catch (Exception ex) {
    ...
    public String getDownloadUrl(String fileName, long expire) {
        try {
            minioClient.statObject(StatObjectArgs.builder().bucket(minioConfig.getBucket())
                    .object(fileName).build());
            if (expire > 0 && expire < (7 * 24 * 3600)) {
                return minioClient.getPresignedObjectUrl(GetPresignedObjectUrlArgs.builder().method(Method.GET)
                        ...
                        .expiry((int) expire, TimeUnit.SECONDS).build());
            } else {
                return minioClient.getPresignedObjectUrl(GetPresignedObjectUrlArgs.builder().method(Method.GET)
                        .bucket(minioConfig.getBucket()).object(fileName).build());
            }
        } catch (Exception e) {
