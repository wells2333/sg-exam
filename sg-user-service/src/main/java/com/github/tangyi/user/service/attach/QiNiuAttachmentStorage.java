    ...
    public String uploadChunk(com.github.tangyi.api.user.attach.ChunkUploadContext context) {
        try {
            ...
            ApiUploadV2UploadPart.Request req = new ApiUploadV2UploadPart.Request(urlPrefix, token,
                    context.getUploadId(), context.getIndex())
                    .setKey(chunkName)
            ...
            throw new QiNiuAttachException(ex, "Failed to upload chunk. uploadId: " + context.getUploadId() + ", index: " + context.getIndex());
        }
    }
    ...
    public Attachment mergeChunks(Attachment prepare, AttachGroup group, List<SysAttachmentChunk> chunks) throws CommonException {
        ...
        try {
            ...
            List<Map<String, Object>> chunksInfo = chunks.stream()
                    .map(chunk -> JSON.parseObject(chunk.getChunkUploadRes()))
                    .collect(Collectors.toList());
            ApiUploadV2CompleteUpload api = new ApiUploadV2CompleteUpload(client);
            ApiUploadV2CompleteUpload.Request req = new ApiUploadV2CompleteUpload
                    .Request(urlPrefix, token, uploadId, chunksInfo)
                    .setKey(key)
                    .setFileName(prepare.getAttachName())
                    .setCustomParam(Collections.emptyMap());
