  ...
  >
    <BasicForm @register="register">
      <template #play="{ model, field }">
        <SgAudioPlayer :option="{ src: model.speechUrl, coverRotate: false }" style="margin-right: 12px;" > </SgAudioPlayer>
      </template>
    </BasicForm>
  </PageWrapper>
</template>
...
export default defineComponent({
  ...
  setup(props) {
    ...
    let subjectId = subjectData !== null ? subjectData.id : undefined;
    if (subjectId) {
    ...
    const schemas: any[] = [];
    schemas.push(...speechSubjectNameSchemas);
