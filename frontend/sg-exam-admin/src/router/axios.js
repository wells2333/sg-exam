...
import { isNotEmpty, isSuccess, isExistVisitView } from '@/utils/util'
import { refreshToken } from '@/api/admin/login'
import { Message } from 'element-ui'
import errorCode from '@/const/errorCode'
import NProgress from 'nprogress' // progress bar
import 'nprogress/nprogress.css'// progress bar style
const whiteList = ['/auth-service/authentication/removeToken']// 白名单
...
axios.interceptors.response.use(data => {
  ...
  if (error.response) {
    ...
    if ((error.response.status === 401 || error.response.status === 403)) {
      ...
      if (isExistVisitView(store.getters.visitedViews, '404')) {
        store.dispatch('delView', {path: '/404/', fullPath: '/404/'})
      }
      router.replace({
        path: '404',
      ...
      // 判断是否已经有500页面
      if (isExistVisitView(store.getters.visitedViews, '500')) {
        store.dispatch('delView', {path: '/500/', fullPath: '/500/'})
      }
      router.replace({
        path: '/500',
        query: { redirect: router.currentRoute.fullPath }
